...
// **************************************  AJAX REQUESTS SECTION *****************************************
  ...
  // Display Subcategories & attributes
      $(document).on('change','#cat',function () {
        ...
        $('#market_id').empty();
        $('#market_id').append(`<option value="">Select Market</option>`);
  ...
  // Display Attributes for Selected Childcategory Starts
      $(document).on('change','#childcat',function () {
        ...
        $.get(getattrUrl + '?id=' + this.value + '&type=childcategory', function(data) {
          ...
          var select_id = $('#childcat').val();
          // alert(base_url)
          $('#market_id').empty();

          $.get(base_url + '/vendor/child-markets/' + select_id, function(result) {
            console.log(result);

            for (var i = 0; i < result.length; i++) {
              $('#market_id').append(`<option value="` + result[i]['id'] + `">` + result[i]['name'] + `</option>`);
            }
