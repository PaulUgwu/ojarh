...
class UserController extends Controller
{
    ...
    public function vendorrequestsub(Request $request)
    {
        ...
        $settings = Generalsetting::findOrFail(1);
                    ...
                    // give agent comission for bringing the vendor
                    if(FacadesAuth::user()->agent_ref_id != null && FacadesAuth::user()->agent_ref_has_subscribed == 'no'){
                        $my_agent = User::find(FacadesAuth::user()->agent_ref_id);

                        if($my_agent){
                            $my_agent->agent_balance += 1500;
                            $my_agent->save();
                        }

                        FacadesAuth::user()->agent_ref_has_subscribed = 'yes';
