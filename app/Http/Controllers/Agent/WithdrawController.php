...
class WithdrawController extends Controller
{
    ...
    {
        $withdraws = Withdraw::where('user_id','=', Auth::guard('web')->user()->id)->where('type','=','agent')->orderBy('id','desc')->get();
        $sign = Currency::where('is_default','=',1)->first();        
        return view('agent.withdraw.index',compact('withdraws','sign'));
    }
    ...
    {
        $sign = Currency::where('is_default','=',1)->first();
        return view('agent.withdraw.create' ,compact('sign'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->amount > 0){
            ...
            if ($from->agent_balance >= $amount){
                ...
                $from->agent_balance = $from->agent_balance - $amount;
                $from->update();
