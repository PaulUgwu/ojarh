...
@section('content')
	...
	@if($adverts->where('position', 'wide-small'))

		<!-- Banner Area One Start -->
		<section class="banner-section">
			<div class="container">
				@foreach($adverts->where('position', 'wide-small')->chunk(2) as $chunk)
					<div class="row">
						@foreach($chunk as $img)
							<div class="col-lg-6 remove-padding">
								<div class="left">
									<a class="banner-effect" href="{{ $img->link }}" target="_blank">
										<img width="" height="" src="{{asset('storage/adverts/'.$img->image)}}" alt="">
									</a>
								</div>
							</div>
						@endforeach
