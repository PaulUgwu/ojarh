										...
										<a href="{{ route('front.product', $prod->slug) }}" class="item">
											...
											<div class="info">
												...
												<h4 class="price">{{ $prod->showPrice() }} <del><small>{{ $prod->showPreviousPrice() }}</small></del></h4>
														<h5 class="name mb-0">{{ $prod->showName() }}</h5>
														<small class="mt-0">{{ App\Models\User::find($prod->user_id)->address ?? '' }}</small>
														<small class="mt-0">{{ App\Models\User::find($prod->user_id)->owner_name ?? '' }}</small>
